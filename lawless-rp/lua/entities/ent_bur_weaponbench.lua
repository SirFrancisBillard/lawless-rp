...
function ENT:GenerateWeapons()
	...
	for k,v in pairs(weapons.GetList()) do
		if v.Base == "weapon_cs_base" then
			if (v.WeaponType ~= "Free" and v.WeaponType ~= "Throwable") or (string.Left(v.ClassName, 4) == "m9k_" and v.Category ~= "M9K Specialties") then
...
function ENT:Think()
	if SERVER then
		if self.Building == true and self.NextTick <= CurTime() then
		...
		local _, Ang = LocalToWorld(Vector(0, 0, 0), Angle(0, CurTime() * 180, 0), self:GetPos(), self:GetAngles())
